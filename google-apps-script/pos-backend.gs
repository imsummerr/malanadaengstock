...
function setupStock() {
  ...
  STOCK_ITEM_SEED.forEach(function (s) {
    ...
    // ต้องเป็น ไม้ หรือ ถุง ไม่งั้นราคาเพี้ยนและเทียบของหายกับ POS ไม่ได้
    row['หน่วยย่อย'] = /^(กรัม|กก\.?|กิโล|กิโลกรัม)$/.test(s[1])
      ? (bag[s[0]] ? 'ถุง' : 'ไม้') : s[1];
    row['หน่วยแพ็ค'] = 'แพ็ค';
    row['หน่วยย่อยต่อแพ็ค'] = PACK_SIZE;               // แพ็คส่งสาขา 10 ไม้/ถุง
    row['ราคาขาย/หน่วยย่อย'] = PRICE_LIST[s[0]] || UNIT_PRICE;
    row['หมายเหตุ'] = s[2];
...
var ITEM_UNITS = {
  ...
  // ตักจากกระปุก ที่ละ 5 กรัม — ไม่ได้แพ็คส่ง ส่งทั้งกระปุก
  'สาหร่ายกระปุก':  { sub: 'ที่',  pack: 'กระปุก', perPack: 1, note: 'ที่ละ 5 กรัม' },

  // 1 ฝักผ่าได้ 2 อัน ขายอันละ 10 — ฝักเป็นเรื่องการเตรียม ไม่ใช่หน่วยส่ง
  // ส่งสาขาเป็นแพ็ค 10 อัน เหมือนตัวอื่น
  'ข้าวโพดฝักใหญ่': { sub: 'อัน', pack: 'แพ็ค', perPack: PACK_SIZE, note: '1 ฝักแบ่ง 2 อัน อันละ 10 บาท' }
};
  ...
  for (var i = 0; i < n; i++) {
    ...
    if (name) {
      ...
      if (!sub || /^(กรัม|กก\.?|กิโล|กิโลกรัม|g|kg)$/i.test(sub)) {
        var want = bag[name] ? 'ถุง' : 'ไม้';
        if (sub !== want) { was.push('หน่วย ' + (sub || 'ว่าง') + '→' + want); sub = want; }
      }
      if (!pk) { pk = 'แพ็ค'; was.push('หน่วยแพ็ค→แพ็ค'); }
      if (per <= 1) { was.push('แพ็ค ' + (per || 'ว่าง') + '→' + PACK_SIZE); per = PACK_SIZE; }
      if (pr <= 0) {
        pr = PRICE_LIST[name] || UNIT_PRICE;
        was.push('ราคา→' + pr);
      }
    }
