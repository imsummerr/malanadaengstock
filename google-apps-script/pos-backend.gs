...
// ── หน่วยของทั้งระบบ ──
// ครัวกลางซื้อของเข้าเป็น "กิโลกรัม" (ลูกชิ้นมาเป็นถุงแต่ก็ชั่งเป็นกิโล)
//   → น้ำหนักกับราคาต่อกิโลอยู่ในชีต "ซื้อของเข้า" ที่บอทไลน์บันทึกให้
// เสียบไม้ / จัดถุงเสร็จแล้วแพ็คไว้ แพ็คละ 10 → ลงชีตสต็อกตรงนี้
// ส่งสาขา 1 แพ็ค = 10 ไม้ หรือ 10 ถุง
// ขายหน้าร้าน 1 ไม้ / 1 ถุง = 10 บาท
//
// ชีตสต็อกจึงนับเป็น "ไม้/ถุง" เสมอ ไม่ใช่กิโลกรัม
// ถ้าตั้งหน่วยย่อยเป็นกรัมไว้ ราคาจะเพี้ยนและเทียบของหายกับ POS ไม่ได้
var PACK_SIZE = 10;      // 1 แพ็ค = กี่ไม้/ถุง
var UNIT_PRICE = 10;     // ขายไม้ละ/ถุงละกี่บาท (ที่ไม่ใช่ 10 อยู่ใน PRICE_LIST)

/** ของที่ขายเป็นถุง/มัด ไม่ได้เสียบไม้ — ที่เหลือเป็นไม้หมด */
var BAG_ITEMS = ['ผักกาดขาว', 'ผักบุ้ง', 'กวางตุ้ง', 'กะหล่ำ',
                 'เห็ดเข็ม', 'เห็ดเข็มทอง', 'เห็ดชิเมจิ',
                 'เส้นมันเทศ', 'เส้นอุด้ง', 'เส้นแก้ว', 'วุ้นเส้น', 'วุ้นเส้นเกาหลี'];
...
function setupStock() {
  ...
  var bag = {};
  BAG_ITEMS.forEach(function (b) { bag[b] = true; });

  var added = 0;
  STOCK_ITEM_SEED.forEach(function (s) {
    if (have[s[0]]) return;
    var u = ITEM_UNITS[itemKey_(s[0])];   // ของที่หน่วยไม่เหมือนชาวบ้าน เช่น กระปุก ท่อน
    var row = {};
    row['สินค้า'] = s[0];
    // seed เขียนหน่วยตอนซื้อเข้า (กรัม) ไว้ แต่ชีตนี้นับหลังเสียบ/จัดถุงแล้ว
    // ต้องเป็นหน่วยที่ขายจริง ไม่งั้นราคาเพี้ยนและเทียบของหายกับ POS ไม่ได้
    row['หน่วยย่อย'] = u ? u.sub
      : (/^(กรัม|กก\.?|กิโล|กิโลกรัม)$/.test(s[1]) ? (bag[s[0]] ? 'ถุง' : 'ไม้') : s[1]);
    row['หน่วยแพ็ค'] = u ? u.pack : 'แพ็ค';
    row['หน่วยย่อยต่อแพ็ค'] = u ? u.perPack : PACK_SIZE;   // แพ็คส่งสาขา 10 หน่วย
    row['ราคาขาย/หน่วยย่อย'] = itemPrice_(s[0]);
    row['หมายเหตุ'] = u ? u.note : s[2];
    appendByCols_(items, map, row);
    added++;
  });
  ...
  Logger.log('ติดตั้งเรียบร้อย — เพิ่มสินค้าใหม่ ' + added + ' รายการ\n' +
             'ตั้งให้แล้ว: 1 แพ็ค = ' + PACK_SIZE + ' ไม้/ถุง · ขายไม้ละ ' + UNIT_PRICE + ' บาท\n' +
             '(ที่ราคาไม่ใช่ ' + UNIT_PRICE + ' ดึงจาก PRICE_LIST ให้แล้ว)\n\n' +
             ...
             'ของเก่าที่มีอยู่แล้วไม่ถูกแตะ — รัน previewPackAndPrice ดูว่าต้องเติมอะไรบ้าง\n' +
...
var ITEM_UNITS = {
  ...
  'ผักกาดขาว':     { sub: 'ถุง', pack: 'แพ็ค', perPack: PACK_SIZE, note: 'จัดเซ็ตใส่ถุง ถุงละ 10 บาท' },
  'ผักบุ้ง':        { sub: 'ถุง', pack: 'แพ็ค', perPack: PACK_SIZE, note: 'จัดเซ็ตใส่ถุง ถุงละ 10 บาท' },
  'กวางตุ้ง':       { sub: 'ถุง', pack: 'แพ็ค', perPack: PACK_SIZE, note: 'จัดเซ็ตใส่ถุง ถุงละ 10 บาท' },
  'เห็ดเข็มทอง':    { sub: 'ถุง', pack: 'แพ็ค', perPack: PACK_SIZE, note: 'จัดเซ็ตใส่ถุง ถุงละ 10 บาท' },

  // เส้นมัดขาย มัดละ 10 · แพ็คส่งสาขา 10 มัด
  'เส้นมันเทศ':     { sub: 'มัด', pack: 'แพ็ค', perPack: PACK_SIZE, note: 'มัดละ 10 บาท' },
  'เส้นอุด้ง':      { sub: 'มัด', pack: 'แพ็ค', perPack: PACK_SIZE, note: 'มัดละ 10 บาท' },
  'เส้นแก้ว':       { sub: 'มัด', pack: 'แพ็ค', perPack: PACK_SIZE, note: 'มัดละ 10 บาท' },

  // ขายเป็นกระปุก · แพ็คส่งสาขา 10 กระปุก
  'สาหร่ายกระปุก':  { sub: 'กระปุก', pack: 'แพ็ค', perPack: PACK_SIZE, note: '1 แพ็ค 10 กระปุก · กระปุกละ 10 บาท' },

  // 1 ฝักผ่าได้ 2 ท่อน ขายท่อนละ 10 — ฝักเป็นเรื่องการเตรียม ไม่ใช่หน่วยส่ง
  // ส่งสาขาเป็นแพ็ค 10 ท่อน เหมือนตัวอื่น
  'ข้าวโพดฝักใหญ่': { sub: 'ท่อน', pack: 'แพ็ค', perPack: PACK_SIZE, note: '1 ฝักแบ่ง 2 ท่อน · ท่อนละ 10 บาท' }
};
...
/** ราคาขายต่อหน่วยย่อยของสินค้าตัวนี้ — ไม่มีในตารางก็ใช้ราคามาตรฐาน */
function itemPrice_(name) {
  return PRICE_LIST[String(name || '').trim()] || PRICE_LIST[itemKey_(name)] || UNIT_PRICE;
}

/** ของที่เลิกขายแล้ว — ลบออกจากชีตรายการสินค้า */
// รากบัวเคยอยู่ในนี้ แต่กลับมาขายอีกแล้ว จึงเอาออก
// ถ้าไม่เอาออก รันฟังก์ชัน removeDiscontinuedItems เมื่อไหร่ มันจะลบทิ้งซ้ำอีก
var ITEM_DISCONTINUED = ['เนื้อแดง', 'หมึก', 'กะหล่ำ'];
...
/**
 * เติม "หน่วยย่อยต่อแพ็ค" กับ "ราคาขาย/หน่วยย่อย" ที่ setupStock เว้นว่างไว้
 *
 * สองช่องนี้ว่างอยู่ = ระบบพัง 2 อย่าง
 *   ราคา 0    → ตีมูลค่าของหายไม่ได้ ไลน์เลยไม่เคยแจ้งของหายสักครั้ง
 *   แพ็ค 1    → "เตือนเมื่อเหลือ 5 แพ็ค" กลายเป็นเหลือ 5 ไม้ เตือนช้าไป 10 เท่า
 *
 * ไม่เขียนทับค่าที่กรอกไว้แล้ว — เติมเฉพาะช่องที่ว่างหรือเป็นค่าเริ่มต้น
 * ยกเว้นหน่วยย่อยที่เป็น "กรัม/กิโลกรัม" ซึ่งเป็นค่าที่ทำให้เทียบของหายไม่ได้
 * ตัวนั้นเขียนทับให้เป็น ไม้ หรือ ถุง เพราะชีตสต็อกนับหลังเสียบ/จัดถุงแล้วเสมอ
 */
function packAndPrice_(dryRun) {
  var sh = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_ITEMS);
  if (!sh) { Logger.log('ไม่พบชีต "' + SHEET_ITEMS + '"'); return; }
  var map = ensureCols_(sh, ITEM_COLS);
  var last = sh.getLastRow();
  ...
  var subs = [], packs = [], perPacks = [], prices = [], changes = [];

  for (var i = 0; i < n; i++) {
    var name = String(vals[i][map['สินค้า']] || '').trim();
    var sub  = String(vals[i][map['หน่วยย่อย']] || '').trim();
    var pk   = String(vals[i][map['หน่วยแพ็ค']] || '').trim();
    var per  = Number(vals[i][map['หน่วยย่อยต่อแพ็ค']]) || 0;
    var pr   = Number(vals[i][map['ราคาขาย/หน่วยย่อย']]) || 0;
    var was = [];

    if (name) {
      var u = ITEM_UNITS[itemKey_(name)];   // ของที่หน่วยไม่เหมือนชาวบ้าน เช่น กระปุก ท่อน

      // กรัม/กิโล = หน่วยตอนซื้อเข้า ไม่ใช่หน่วยที่นับในสต็อก
      if (!sub || /^(กรัม|กก\.?|กิโล|กิโลกรัม|g|kg)$/i.test(sub)) {
        var want = u ? u.sub : (bag[name] ? 'ถุง' : 'ไม้');
        if (sub !== want) { was.push('หน่วย ' + (sub || 'ว่าง') + '→' + want); sub = want; }
      }
      var wantPack = u ? u.pack : 'แพ็ค';
      if (!pk) { pk = wantPack; was.push('หน่วยแพ็ค→' + wantPack); }
      if (per <= 1) {
        var wantPer = u ? u.perPack : PACK_SIZE;
        was.push('แพ็ค ' + (per || 'ว่าง') + '→' + wantPer);
        per = wantPer;
      }
      if (pr <= 0) {
        pr = itemPrice_(name);
    ...
    subs.push([sub]); packs.push([pk]); perPacks.push([per || '']); prices.push([pr || '']);
    if (was.length) changes.push('• ' + name + '  ' + was.join(' · '));
  }

  if (!changes.length) { Logger.log('ครบอยู่แล้ว ไม่มีอะไรต้องเติม'); return; }

  if (dryRun) {
    Logger.log('จะแก้ ' + changes.length + ' รายการ (ยังไม่ได้เขียนลงชีต):\n' + changes.join('\n') +
               '\n\nพอใจแล้วรัน applyPackAndPrice');
    return;
  }

  sh.getRange(2, map['หน่วยย่อย'] + 1, n, 1).setValues(subs);
  sh.getRange(2, map['หน่วยแพ็ค'] + 1, n, 1).setValues(packs);
  sh.getRange(2, map['หน่วยย่อยต่อแพ็ค'] + 1, n, 1).setValues(perPacks);
  sh.getRange(2, map['ราคาขาย/หน่วยย่อย'] + 1, n, 1).setValues(prices);
  Logger.log('เติมแล้ว ' + changes.length + ' รายการ:\n' + changes.join('\n'));
}

/** ดูก่อนว่าจะแก้อะไรบ้าง ไม่เขียนลงชีต */
function previewPackAndPrice() { packAndPrice_(true); }

/** เติมจริง — 1 แพ็ค = 10 ไม้/ถุง · ขายไม้ละ 10 บาท */
function applyPackAndPrice() { packAndPrice_(false); }
