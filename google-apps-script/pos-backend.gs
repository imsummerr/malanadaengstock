...
var PACK_COLS = ['วันที่เวลา', 'สาขา', 'ผู้ตรวจ',
                 ...
                 'ชั่งได้เนื้อ', 'ชั่งได้เนื้อ2', 'ชั่งได้เนื้อ3'];
 ...
 */
/** ของแพ็คตัวนี้ใช้วัตถุดิบตัวนี้ได้ไหม — ตรงตัว หรืออยู่ในกลุ่มที่ตั้งไว้ */
function rawAllowed_(out, name) {
  var list = (out && out.raws) || [];
  if (!list.length) return true;              // ยังไม่ได้ตั้งสูตร ปล่อยผ่าน
  for (var i = 0; i < list.length; i++) {
    if (list[i] === name) return true;
    var g = rawGroupOf_(list[i]);
    if (g && g.indexOf(name) !== -1) return true;
  }
  return false;
}
...
function handleStockPack_(body) {
  ...
  var auto = out.autoRaws || {};   // ชื่อกลุ่มไม่มีอัตราส่วน จึงไม่หลุดมาทางนี้
  ...
  for (var i = 0; i < want.length; i++) {
    ...
    // เลือกมาจากกลุ่มได้ แต่ต้องเป็นตัวที่อยู่ในกลุ่มจริง ไม่ใช่อะไรก็ได้
    if (!rawAllowed_(out, nm)) {
      return { success: false, message: '"' + out.name + '" ใช้ "' + nm + '" ไม่ได้' };
    }
    var q = Number(want[i].qty) || 0;
    if (!(q > 0)) return { success: false, message: 'กรอกว่าใช้ "' + nm + '" ไปเท่าไหร่' };
    // q = เอาออกมาจากสต็อกกี่โล → ตัดเท่านี้ ยอดลงศูนย์พอดี ไม่มีน้ำแข็งค้าง
    // net = ละลายแล้วชั่งได้เนื้อเท่าไหร่ → เก็บไว้ดู yield เฉย ๆ ไม่ตัดซ้ำ
    var net = Number(want[i].net) || 0;
    if (net && net > q) {
      return { success: false,
               message: '"' + nm + '" ได้เนื้อ ' + net + ' แต่เอามาแค่ ' + q +
                        ' — เนื้อต้องไม่เกินที่เอามา' };
    }
    used.push({ item: it, qty: q, net: net, loss: net ? round_(q - net) : 0 });
  }
            ...
            (used.length
              ? 'ใช้ ' + used.map(function (u) {
                  return u.item.name + ' ' + u.qty + ' ' + u.item.subUnit +
                         (u.loss > 0 ? '  (ได้เนื้อ ' + u.net + ' · น้ำแข็ง ' + u.loss + ')' : '');
                }).join('\n    ') + '\n'
              : '') +
...
function handleStockBootstrap_(p) {
  ...
  return { success: true, data: {
    ...
    rawGroups: (function () {
      var g = {};
      Object.keys(RAW_GROUPS).forEach(function (k) { g[k] = RAW_GROUPS[k].map(rawName_); });
      return g;
    })()
  } };
}
...
var PACK_SIZE_EXCEPTION = {
  ...
  'ฟองเต้าหู้ม้วน': 12,
  'น้ำจิ้มงา (กระปุก)': 50,
  'น้ำจิ้มสุกี้ (กระปุก)': 50,
  // ผสมแล้วนับเป็นกรัมล้วน ไม่มีชั้นแพ็ค จะได้ไม่ต้องคิดว่ากี่กรัมเป็น 1 แพ็ค
  'หม่าล่า(ผสมแล้ว)': 1,
  // ส่งสาขาเป็นถุง ถุงละ 900 มล.
  'นมข้นจืด': 900
};
 ...
 */
var RAW_KG = [
  // หมูซื้อมาแบบหั่นเสร็จแล้ว ตอนสั่งก็รู้แล้วว่าจะสไลซ์หรือหั่นชิ้น
  // เลยเก็บแยกกันตั้งแต่ของดิบ ไม่ได้มีก้อนกลางให้เลือกทีหลัง
  'หมูสามชั้นสไลซ์', 'สันคอสไลซ์', 'สันนอกสไลซ์', 'สะโพกหมูสไลซ์',
  'หัวไหล่หมูติดหนังสไลซ์', 'หัวไหล่หมูติดหนังหั่นชิ้น',
  'อกไก่เนื้อล้วนติดหนัง',
  'ผักกาดขาว', 'ผักบุ้ง', 'กวางตุ้ง', 'กะหล่ำ', 'มันฝรั่ง', 'ฟักทอง', 'รากบัว',
...
var RAW_BAG = { 'มันเทศ': 'อัน',
                'น้ำจิ้มงา': 'ถุง', 'น้ำจิ้มสุกี้': 'กิโลลิตร' };

/**
 * ของดิบที่ซื้อมาเป็นแพ็ค แล้วนับย่อยลงไปได้อีกชั้น
 * ปกติของดิบไม่มีชั้นแพ็ค (1 ถุงคือ 1 ถุง) แต่บางอย่างซื้อยกแพ็คมา
 * แล้วเวลานับสต็อกอยากนับทั้งแบบแพ็คและแบบชิ้น เช่นเหลือ 2 แพ็ค 5 อัน
 * ใส่ไว้ตรงนี้แล้วทั้งของดิบและของแพ็คจะใช้ขนาดแพ็คเดียวกัน
 */
var RAW_PACK = {
...
/**
 * กลุ่มวัตถุดิบที่เลือกได้ — ของที่พันไม่ได้ใช้หมูชิ้นเดิมทุกครั้ง
 * แล้วแต่วันนั้นมีอะไร ต้องให้คนแพ็คเลือกเองว่าใช้หมูอะไรไปเท่าไหร่
 * เขียนในช่องวัตถุดิบว่า "@เนื้อหมู" = เลือกจากกลุ่มนี้
 */
var RAW_GROUPS = {
  // พันด้วยหมูก้อนไม่ได้ ต้องสไลซ์ก่อน เลยเลือกจากหมูที่สไลซ์มาแล้ว
  'เนื้อหมู': ['หมูสามชั้นสไลซ์', 'สันคอสไลซ์', 'สันนอกสไลซ์',
              'หัวไหล่หมูติดหนังสไลซ์', 'สะโพกหมูสไลซ์']
};
var RAW_GROUP_MARK = '@';

/** ชื่อในกลุ่ม พร้อมคำว่า (ดิบ) ต่อท้าย — ไม่ใช่กลุ่มก็คืนค่าว่าง */
function rawGroupOf_(name) {
  var t = String(name || '').trim();
  if (t.charAt(0) !== RAW_GROUP_MARK) return null;
  var g = RAW_GROUPS[t.slice(1)];
  return g ? g.map(rawName_) : null;
}

var STICK_ITEMS = [
  // เนื้อที่ใช้พันเลือกเอาตอนแพ็ค ส่วนไส้ในเป็นตัวเดิมเสมอ
  ['หมูพันเห็ดเข็มทอง',   ['@เนื้อหมู', 'เห็ดเข็ม'],    ''],
  ['หมูพันสาหร่าย',       ['@เนื้อหมู', 'สาหร่าย'],     ''],
  ['หมูพันเห็ดชิเมจิ',     ['@เนื้อหมู', 'เห็ดชิเมจิ'],   ''],
  ['หมูพันผักกาดขาว',     ['@เนื้อหมู', 'ผักกาดขาว'],   ''],
  // หมูสไลซ์ ของดิบชื่อเดียวกัน แค่ยังไม่ได้แบ่งเป็นแพ็ค
  ['สันนอกสไลซ์',           '',        ''],
  ['หมูสามชั้นสไลซ์',        '',        ''],
  ['สะโพกหมูสไลซ์',          '',        ''],
  ['หัวไหล่หมูติดหนังสไลซ์',   '',        ''],
  ['หัวไหล่หมูติดหนังหั่นชิ้น', '',        ''],
  ['อกไก่เนื้อล้วนติดหนัง',    '',        ''],
  ['ดอลลี่',              '',            ''],
  ['แมงกะพรุน',           '',            ''],
  ['ปลาหมึกกรอบ',         '',            ''],
...
/**
 * ของที่ครัวกลางเทใส่ภาชนะแล้วส่งไปสาขา
 *
 * ต่างจากของแพ็คตัวอื่นตรงที่วัตถุดิบเป็น "ของใช้ที่มีอยู่แล้ว" หลายตัว
 * ไม่ใช่ของดิบชื่อเดียวกัน — น้ำจิ้มหนึ่งกระปุกกินทั้งน้ำจิ้มและตัวกระปุก
 * ต้นทุนต่อกระปุกจึงเป็นผลรวมของสองอย่าง ระบบคิดให้เองตอนบันทึกแพ็คของ
 *
...
var FILL_ITEMS = [
  ...
  // ผสม 2 : 1 : 1 — กรอกว่าผสมได้กี่กรัม ระบบตัดของให้เองตามสัดส่วน
  // 400 กรัม → เบส 200 · ผง 100 · พริกป่น 100
  ['หม่าล่า(ผสมแล้ว)', 'กรัม',
   ['เบสหม่าล่า x0.5', 'ผงหม่าล่า x0.25', 'พริกป่น x0.25'], 0,
   'ผสม 2:1:1 — เบส 2 ส่วน ผง 1 ส่วน พริกป่น 1 ส่วน · กรอกแค่ว่าได้กี่กรัม'],
  // 1 ถุง = 900 มล. ชงจากนมผง 300 กรัม + น้ำร้อน 900 มล.
  // เก็บเป็น มล. เลยหารเป็น 300/900 = 0.333333333 กรัมต่อ 1 มล.
  // เขียนทศนิยมยาว ๆ ไว้ให้ 1 ถุงตัดได้ 300 กรัมพอดี ไม่ขาดไม่เกิน
  // ครัวกลางถือนมผง สาขาถือนมข้นจืด คนละชื่อเพราะคนละสภาพของ
  ['นมข้นจืด', 'มล.', ['นมผง x0.333333333'], 0,
   'ถุงละ 900 มล. — ชงจากนมผง 300 กรัม + น้ำร้อน 900 มล. · กรอกแค่ว่าได้กี่ถุง']
];

/** หน่วยแพ็คของของที่เทใส่ภาชนะ — ไม่ได้ใส่ = "แพ็ค" */
var FILL_PACK_UNIT = { 'นมข้นจืด': 'ถุง' };
...
function itemCatalogue_() {
  ...
  function addPacked(row, subUnit) {
    ...
    // ชื่อที่ขึ้นต้นด้วย @ คือกลุ่ม ต้องงอกของดิบทุกตัวในกลุ่ม
    bases.forEach(function (b) {
      if (String(b).charAt(0) === RAW_GROUP_MARK) {
        (RAW_GROUPS[String(b).slice(1)] || []).forEach(function (x) { rawSeen[x] = true; });
      } else {
        rawSeen[b] = true;
      }
    });
  ...
  // ของที่เทใส่ภาชนะ — วัตถุดิบมีสองแบบปนกัน
  ...
  FILL_ITEMS.forEach(function (r) {
    ...
    out.push({
      name: r[0], kind: KIND_PACKED,
      subUnit: r[1], packUnit: FILL_PACK_UNIT[r[0]] || 'แพ็ค',
      perPack: PACK_SIZE_EXCEPTION[r[0]] || PACK_SIZE,
      perStick: 1, price: Number(r[3]) || 0,
      scope: '', raws: r[2].slice(), note: r[4] || ''
    });
  });
  ...
  Object.keys(rawSeen).forEach(function (base) {
    var pk = RAW_PACK[base];
    var u  = pk ? pk.unit : rawUnitOf_(base);
    out.push({
...
var ITEM_RENAME = {
  ...
  'สามชั้นไม้':          'หัวไหล่หมูติดหนังสไลซ์',
  'หมูไม้':              'สันนอกสไลซ์',
  'หัวไหล่หมูเสียบไม้':   'หัวไหล่หมูติดหนังสไลซ์',
  'สันนอกเสียบไม้':      'สันนอกสไลซ์',
  // ── หมูแยกตามวิธีหั่นตั้งแต่ซื้อ ของดิบก้อนกลางเลิกใช้แล้ว ──
  // ยอดเก่าเป็น 0 อยู่แล้ว ชี้มาที่สไลซ์เพื่อให้อ่านประวัติเก่ารู้เรื่อง
  'หัวไหล่หมู (ดิบ)':     'หัวไหล่หมูติดหนังสไลซ์ (ดิบ)',
  'หัวไหล่หมูติดหนัง (ดิบ)': 'หัวไหล่หมูติดหนังสไลซ์ (ดิบ)',
  'หมูสามชั้น (ดิบ)':     'หมูสามชั้นสไลซ์ (ดิบ)',
  'สันคอ (ดิบ)':         'สันคอสไลซ์ (ดิบ)',
  'สันนอก (ดิบ)':        'สันนอกสไลซ์ (ดิบ)',
  'สะโพกหมู (ดิบ)':      'สะโพกหมูสไลซ์ (ดิบ)',
  // ของแพ็คที่ยุบทิ้ง — แบบถุงกับแบบไม้รวมเป็นตัวเดียว
  'หัวไหล่หมูสไลซ์ (ไม้)': 'หัวไหล่หมูติดหนังสไลซ์',
  'หัวไหล่หมูสไลซ์ (ถุง)': 'หัวไหล่หมูติดหนังสไลซ์',
  'สามชั้นสไลซ์':        'หมูสามชั้นสไลซ์',
  'อกไก่':               'อกไก่เนื้อล้วนติดหนัง',
  'สามชั้นพันเห็ดชิเมจิ':  'หมูพันเห็ดชิเมจิ',
  'พันชิเมจิ':            'หมูพันเห็ดชิเมจิ',
  'สามชั้นพันผักกาดขาว':  'หมูพันผักกาดขาว',
  'พันผักกาดขาว':         'หมูพันผักกาดขาว',
  ...
  'สันคอ (โล)':          'สันคอสไลซ์ (ดิบ)',
  'สันนอก (โล)':         'สันนอกสไลซ์ (ดิบ)',
  'หัวไหล่หมู (โล)':      'หัวไหล่หมูติดหนังสไลซ์ (ดิบ)',
  'หมูสามชั้น (โล)':      'หมูสามชั้นสไลซ์ (ดิบ)',
  'อกไก่ (โล)':          'อกไก่เนื้อล้วนติดหนัง (ดิบ)',
  ...
  // น้ำจิ้มย้ายจากของใช้มาเป็นของดิบ เพราะต้องเทใส่กระปุกก่อนถึงส่งร้านได้
  'น้ำจิ้มงา':            'น้ำจิ้มงา (ดิบ)',
  'น้ำจิ้มสุกี้':          'น้ำจิ้มสุกี้ (ดิบ)',
  // เปลี่ยนจาก "สไลด์" เป็น "สไลซ์" ทั้งหมด ชื่อเก่าแมปมาที่ชื่อใหม่ ประวัติไม่ขาด
  'สะโพกหมูสไลด์':       'สะโพกหมูสไลซ์',
  'สะโพกหมูสไลซ':        'สะโพกหมูสไลซ์',
  'สามชั้นสไลด์':         'หมูสามชั้นสไลซ์',
  'สันนอกสไลด์':         'สันนอกสไลซ์',
  'สันนอกไลซ์':          'สันนอกสไลซ์',
  'สันคอสไลด์':          'สันคอสไลซ์',
  'หัวไหล่หมูสไลด์ (ไม้)': 'หัวไหล่หมูติดหนังสไลซ์',
  'หัวไหล่หมูสไลด์ (ถุง)': 'หัวไหล่หมูติดหนังสไลซ์',
  'หัวไหล่หมูติดหนังสไลด์': 'หัวไหล่หมูติดหนังสไลซ์',
  // ของดิบกับของแพ็คใช้ชื่อเดียวกัน เหมือนหมูทุกตัว
  'อกไก่ (ดิบ)':           'อกไก่เนื้อล้วนติดหนัง (ดิบ)',
  'อกไก่ติดหนัง (ดิบ)':     'อกไก่เนื้อล้วนติดหนัง (ดิบ)'
  ...
  locs.forEach(function (loc) {
    ...
    Object.keys(m).forEach(function (item) {
      ...
      out.push({
        ...
        // แยกประเภทจากการนับจริง — การล้างยอดไม่ใช่ "ของถูกใช้ไป"
        // ถ้าใช้ประเภทเดียวกัน เครื่องคิดต้นทุนจะนับของที่ล้างทิ้งเป็นค่าใช้จ่ายวัตถุดิบ
        'ประเภท': 'ล้างยอด',
...
var SUPPLY_ITEMS = [
  // ── เฉพาะครัวกลาง ──
  // นับเป็นกรัม ไม่ใช่ถุง เพราะหม่าล่าผสมตัดตามสูตรเป็นกรัม
  // ลงเป็นถุงก็ได้ ระบบคูณให้ตามช่อง "หน่วยย่อยต่อแพ็ค"
  ['เบสหม่าล่า',    'กรัม', SCOPE_CENTRAL, 1000, 'ถุง', 'ถุงละ 1000 มล ≈ 1000 กรัม'],
  ['ผงหม่าล่า',     'กรัม', SCOPE_CENTRAL, 100,  'ถุง', 'ถุงละ 100 กรัม'],
  // ยังไม่รู้ว่าถุงละกี่กรัม ตั้งไว้ 1 ก่อน = ต้องชั่งกรัมมาลง
  // ลงเป็น "ถุง" ระบบจะเตือนว่าหน่วยไม่ตรง ดีกว่าเดาแล้วต้นทุนเพี้ยน
  ...
  // ซื้อนมผงเข้าครัวกลาง ชงเป็นนมข้นจืดแล้วค่อยส่งสาขา
  ...
  ['ช้อน',          'อัน',    '',  100, 'แพ็ค', 'แพ็คละ 100 อัน'],
